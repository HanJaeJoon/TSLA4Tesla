...
- [x] **Step 3: 워크플로 재실행 후 artifact 내려받기**

Actions > `Android Build` > `Run workflow` (브랜치 `dev/jjhan-adapp-kit`).

완료 후 실행 결과 페이지 하단의 `app-release-aab` artifact를 내려받아 압축을 푼다.

- [x] **Step 4: Play Console 내부 테스트에 업로드**
...
- [x] **Step 2: checkout이 태그를 가져오도록 수정**
...
- [x] **Step 5: versionCode를 올려 통과 확인 및 스펙 반영**
