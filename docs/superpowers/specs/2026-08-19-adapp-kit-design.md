...
주의: `workflow_dispatch` 워크플로는 **기본 브랜치에 파일이 있어야** Actions UI 목록에 노출된다. main 병합 전에는 작업 브랜치 push 트리거를 임시로 추가해야 하며, 병합 시 그 블록을 제거한다.
...
- **versionCode**: `app.json`의 `android.versionCode`에 명시적으로 유지 (`eas.json`이 `appVersionSource: "local"`). CI는 "마지막 출시본보다 큰가"만 검사한다. `github.run_number` 자동 증가는 로컬 빌드와 어긋나 Play가 거부할 수 있어 쓰지 않는다.
  ...
  검사 기준은 `vc<N>` 형식의 git 태그로 기록한 출시 이력이다. 태그 push 트리거는 이 가드와 충돌하므로(태그를 밀면 그 빌드가 스스로 실패) 쓰지 않는다. 2026-08-19에 가드가 실제로 차단하는지 확인했다 - versionCode 7 + 태그 vc7 상태에서 빌드가 **29초 만에 실패**했다. 빌드 25분을 소모하기 전에 막힌다.
...
- **Play 비공개 테스트 요건의 적용 범위**: 2026-08-19에 내부 테스트 트랙으로 versionCode 7을 출시하는 동안 "테스터 12명 x 14일" 안내가 **전혀 표시되지 않았다.** 계정 단위 관문을 이미 통과한 것으로 보인다. 다만 이는 내부 테스트 트랙 기준 관찰이며, **신규 앱을 프로덕션에 올릴 때도 요건이 없는지는 아직 확인되지 않았다.** Phase 4(대출 계산기) 출시 때 최종 확인한다.

## 확인된 개선 후보

Phase 1 진행 중 관찰한 것으로, kit이나 향후 작업에 반영할 항목이다.

- **R8 축소 활성화**: AAB가 62MB이며 Play가 두 가지로 지적한다 - 업로드 시 "APK 크기를 눈에 띄게 증가시킵니다" 경고, 프로덕션 권장 조치의 "R8 최적화로 앱의 메모리 및 성능 개선". Expo 기본 설정이 R8 축소를 끄고 있어 난독화 매핑 파일도 생성되지 않는다. 앱 크기는 설치 전환율에 직결되고 그것이 곧 광고 노출이므로, kit 차원에서 R8을 켜는 것을 검토한다. 단 RN/네이티브 모듈과의 호환성 검증이 필요하므로 별도 작업으로 다룬다.
- **Gradle 캐시가 작업 브랜치에서 동작하지 않음**: `gradle/actions/setup-gradle`는 기본 브랜치에서만 캐시를 저장하고 그 외 브랜치에서는 읽기 전용이다. main에 캐시가 없으면 읽을 것도 없어 매 빌드가 25분 걸린다(빌드 #1/#2/#3 모두 25분으로 동일). **main 병합 후 첫 빌드에서 캐시가 생성되고 그 다음부터 단축된다.**
